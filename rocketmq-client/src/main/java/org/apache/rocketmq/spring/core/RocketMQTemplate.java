...
public class RocketMQTemplate extends AbstractMessageSendingTemplate<String> implements InitializingBean, DisposableBean {
    ...
    public SendResult sendMessageInTransactionV2(final String destination,
                                                 final Message<?> message) throws MessagingException, MQClientException {
        ...
        if (rocketMsg.getDelayTimeLevel() != 0) {
            MessageAccessor.clearProperty(rocketMsg, MessageConst.PROPERTY_DELAY_TIME_LEVEL);
        }

        Validators.checkMessage(rocketMsg, this.producer);
        SendResult sendResult = null;
        MessageAccessor.putProperty(rocketMsg, MessageConst.PROPERTY_TRANSACTION_PREPARED, "true");
        MessageAccessor.putProperty(rocketMsg, MessageConst.PROPERTY_PRODUCER_GROUP, this.producer.getProducerGroup());
        try {
        ...
        if (sendResult.getSendStatus().equals(SendStatus.SEND_OK)) {
            try {
                if (sendResult.getTransactionId() != null) {
                    rocketMsg.putUserProperty("__transactionId__", sendResult.getTransactionId());
                }
                String transactionId = rocketMsg.getProperty(MessageConst.PROPERTY_UNIQ_CLIENT_MESSAGE_ID_KEYIDX);
                if (null != transactionId && !"".equals(transactionId)) {
                    rocketMsg.setTransactionId(transactionId);
                }
                if (transactionListener != null) {
                    log.debug("Used new transaction API");
                    LocalTransactionState localTransactionState = transactionListener.executeLocalTransaction(rocketMsg, sendResult);
                    if (LocalTransactionState.COMMIT_MESSAGE.equals(localTransactionState)) {
                        getProducer().getDefaultMQProducerImpl().endTransaction(sendResult, localTransactionState, null);
                    }
                }
            } catch (Throwable e) {
                log.info("executeLocalTransactionBranch exception", e);
                log.info(rocketMsg.toString());
            }
        }
        return sendResult;
    }

    public void submitTransactionV2(SendResult sendResult, LocalTransactionState state) throws InterruptedException, UnknownHostException, RemotingException, MQBrokerException {
        getProducer().getDefaultMQProducerImpl().endTransaction(sendResult, state, null);
    }
